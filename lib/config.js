export const configTemplate = {
  ...
  BOT_LANGUAGE: "en",
  DATE_FORMAT: "auto",
  // Time-of-day display format used by formatTime() in utils/dateFormat.js.
  // "auto" → 24h for de/sv, 12h (AM/PM) for en.
  TIME_FORMAT: "auto",
  ...
  // Header for poller notifications of content added directly to Jellyfin
  // (not via Seerr). Empty = use BOT_LANGUAGE default.
  NOTIF_TITLE_JELLYFIN_NEW: "",
  ...
  HEALTH_ALERT_INTERVAL_SECONDS: "120",
  ...
  POST_HELP_CHANNEL_ID: "",
  // When true, MEDIA_APPROVED and MEDIA_AUTO_APPROVED events are sent only as
  // DMs to the requester — no public channel post. Reduces noise for approval events.
  ...
  CLEANUP_FETCH_TIMEOUT_SECONDS: "60",   // per-page Jellyfin timeout: 30 (~500), 60 (~2000), 120 (~6000)
  TMDB_API_KEY: "",
  OMDB_API_KEY: "",
  JELLYFIN_BASE_URL: "",
  JELLYFIN_API_KEY: "",
  ...
  SUBSCRIPTION_POLL_INTERVAL_MINUTES: "60",
  WEEKLY_RECOMMENDATION_DAY: "sunday",
  WEEKLY_RECOMMENDATION_TIME: "18:00",
  ...
  DEFAULT_SERVER_TV: "",
  // Master switch for all poster/backdrop images on Discord embeds (true = show).
  EMBED_SHOW_IMAGES: "true",
  ...
  // How often Questorr polls Jellyfin for newly added items (seconds, 0 = disabled, default: 120)
  JELLYFIN_POLL_INTERVAL_SECONDS: "120",
  ...
  // Round 10: Only notify items added to Jellyfin within the last N days (matches Jellyfin's
  // "Recently Added" concept). Items older than this are NEVER sent to Discord, even on full-scan.
  // Set to 0 to disable the filter entirely (legacy/power-user mode — DO NOT do this casually
  // as it allows the "Jetzt prüfen" button to notify the entire library).
  JELLYFIN_RECENT_ADDED_DAYS: "7",
  // Round 11: When "true", Questorr triggers Jellyfin's built-in library scan (POST /Library/Refresh)
  // before each poll cycle. This ensures newly added filesystem content is indexed by Jellyfin
  // before Questorr queries for recent items — eliminating detection latency without needing
  // the Jellyfin webhook plugin. Throttled to once per 60 seconds. Set to "false" to disable.
