...
/**
 * Fetch personalized recommendations from Jellyfin's built-in recommendation engine.
 *
 * Calls `GET /Movies/Recommendations?userId=<id>` which returns recommendations
 * grouped by category — e.g. "SimilarToRecentlyPlayed", "HasDirectorFromRecentlyPlayed",
 * "HasActorFromRecentlyPlayed", each with a `BaselineItemName` (the source item/person).
 *
 * Results are flattened, deduplicated by item Id, and the source baseline is preserved
 * as a "Because you watched X" hint per item.
 *
 * Movies only — Jellyfin's recommendation engine is movie-focused. Empty array if
 ...
 * @param {string} baseUrl - Jellyfin base URL
 * @param {object} [opts]
 * @param {number} [opts.itemLimit=8]     - Max items per category
 * @param {number} [opts.categoryLimit=5] - Max categories
 * @param {number} [opts.totalLimit=10]   - Max total items returned
 ...
 * }>>}
 */
export async function fetchJellyfinRecommendations(
  jellyfinUserId,
  apiKey,
  baseUrl,
  { itemLimit = 8, categoryLimit = 5, totalLimit = 10 } = {}
) {
  try {
    const safeBase = new URL(baseUrl);
    ...
    for (const category of categories) {
      const baseline = category?.BaselineItemName || null;
    ...
    logger.warn(`[Jellyfin] fetchJellyfinRecommendations error: ${err?.message || err}`);
    return [];
  }
}
