...
const level = () => {
  ...
  // LOG_LEVEL env var allows fine-tuning: error | warn | info | verbose | debug
...
logger.seerr = (action, message) => {
  logger.info(`[Seerr] ${action}: ${message}`);
};

logger.seerrWebhook = (event, subject, detail = "") => {
  logger.info(`[SEERR WEBHOOK] ${event} | "${subject}"${detail ? " | " + detail : ""}`);
};
