...
Questorr is developed on the `main` branch, and security fixes are only guaranteed for the latest release and the current `main` branch.
...
- Direct message via the official [JellyForge](https://discord.gg/J9V9ySZQKf) Discord server
...
- Keep Questorr and all dependencies up to date
...
This policy applies to vulnerabilities that could affect the confidentiality, integrity, or availability of Questorr or user data, including but not limited to:
...
- Issues caused solely by insecure third-party deployment choices without an Questorr defect
