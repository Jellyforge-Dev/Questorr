...
| Slash commands | `bot/commands/*.js` | One file per command: `search`, `status`, `random`, `recommend`, `discover`, `collection`, `cast`, `similar`, `watchlist`, `history`, `foryou`, `upcoming`, `queue`, `subscribe`, `report`, `help` (`trending` is handled inline in `bot/interactions.js`) |
...
| Pollers/daily jobs | `bot/jellyfinPoller.js`, `bot/seerrStatusPoller.js`, `bot/subscriptionPoller.js`, `bot/healthAlertPoller.js`, `bot/dailyPick.js`, `bot/cleanupAdvisor.js`, `bot/weeklyDigest.js`, `bot/weeklyRecommendation.js` | Background polling for Jellyfin/Seerr state and scheduled daily/weekly posts |
...
| API clients | `api/seerr.js`, `api/tmdb.js`, `api/jellyfin.js`, `api/omdb.js` | External API wrappers |
